...
if(isset($_POST['update'])){
   ...
   $old_image_06 = $_POST['old_image_06'];
   ...
   if(!empty($image_06)){
      if($image_size_06 > 2000000){
         $message[] = 'image size is too large!';
      }else{
         $update_image_06 = $conn->prepare("UPDATE `products` SET image_06 = ? WHERE id = ?");
         $update_image_06->execute([$image_06, $pid]);
         move_uploaded_file($image_tmp_name_06, $image_folder_06);
         unlink('../uploaded_img/'.$old_image_06);
         $message[] = 'image 06 updated successfully!';
      }
   }
